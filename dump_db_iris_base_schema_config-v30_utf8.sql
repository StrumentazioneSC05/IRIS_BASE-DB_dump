...
COMMENT ON COLUMN animazione_webgis.delta_inizio IS 'in delta_udm specificare il tempo di partenza del delta_min. normalmente ├¿ 00 ma potrebbe essere altro...';
...
COMMENT ON COLUMN news_message.data_scadenza IS 'inserire la data di scadenza quando non si vuole far piu'' comparire il messaggio';
...
COMMENT ON COLUMN webgis_popups.grafici_stazione IS 'per decidere a quale tipo di grafico puntare. I valori saranno vincolati dal codice. In particolare sono gli IF all''interno della funzione "grafici_rete", in js_functions.js.
...
COPY animazione_webgis (label, nome_file, tipo_dato, formato_data, campo_data, gid, note, delta_min, delta_max, delta_udm, path, data_index, bounds3857, size, delta_inizio, delta_passo, attivo, webgis_idx_arr, frame_offset) FROM stdin;
...
SELECT pg_catalog.setval('animazione_webgis_gid_seq', 27, true);
...
COPY anomalie_severita (severita_idx, severita_descr, impatto_operativo, severita_colore) FROM stdin;
1	lieve	t	yellow
2	moderata	t	orange
3	grave	t	red
11	lieve	f	khaki
...
COPY anomalie_sistemi (id_sistema, descrizione, data_agg, autore_agg) FROM stdin;
...
SELECT pg_catalog.setval('news_message_gid_seq', 18, true);
...
COPY webgis_base_layers (baselayer_idx, openlayer_name, layer_legend_name, layer_description, geom_srid) FROM stdin;
...
SELECT pg_catalog.setval('webgis_base_layers_baselayer_idx_seq', 16, true);
...
COPY webgis_groups (legend_group_name, pedice, group_father, description, group_idx, order_in_webgis, expanded, is_father, children_array) FROM stdin;
...
SELECT pg_catalog.setval('webgis_groups_group_idx_seq', 31, true);
...
SELECT pg_catalog.setval('webgis_indici_idx_seq', 16, true);
...
SELECT pg_catalog.setval('webgis_ol_layers_layer_idx_seq', 220, true);
...
COPY webgis_query_map (query_map_idx, query_description) FROM stdin;
...
COPY webgis_target (webgis_idx, radius, udm, the_geom) FROM stdin;
...
COPY webgis_toolbar_tools (tool_idx, tool_name, tool_description, tool_icon) FROM stdin;
...
SELECT pg_catalog.setval('webgis_toolbar_tools_tool_idx_seq', 17, true);
